...
const PendientesSuper = ({ actualizarContador }) => {
    ...
    const fetchSolicitudes = async () => {
        try {
            setLoading(true);
            const response = await axios.get("https://popnocturna.vercel.app/api/lugares/pendientes", {
                headers: {
            ...
            const usuariosResponse = await axios.get("https://popnocturna.vercel.app/api/usuarios", {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    if (loading) {
        return (
            <div className="pendientes-super-container">
                <h2 className="pendientes-super-title">Solicitudes de Creación de Lugar</h2>
                    ...
                    <tbody>
                        {solicitudes.map(solicitud => (
                            <tr key={solicitud.id} className="pendientes-super-row">
                                ...
                                <td className={`pendientes-super-status ${solicitud.aprobacion ? 'active' : 'inactive'}`}>
